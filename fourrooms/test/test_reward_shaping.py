...
@pytest.fixture
def reward_shaping():
    kwargs = {"discount":0.99, "eta":0.5, "subgoal_serieses":[[25, 65]], "nfeatures":1}
    return CumulativeSubgoalRewardWithPenalty(**kwargs)

def test_at_subgoal(reward_shaping):
    assert reward_shaping.at_subgoal(25) == True
    assert reward_shaping.at_subgoal(26) == False


def test_init_subgoals(reward_shaping):
    assert reward_shaping.init_subgoals([[25, 65]]) == {25:(0, 0)}
